...
Since the dataset shows average rent by state, it may be more informative to look at the data geographically. We use ggplot geom_map to do this.

```{r extension1}
state = tolower(avg.rent.bystate$State_Name)
map <- map_data("state")
l <- ggplot(avg.rent.bystate, aes(fill = AvgRent))
l + geom_map(aes(map_id = state), map = map) +
expand_limits(x = map$long, y = map$lat) + theme(axis.text.x = element_text(angle = 90)) + xlab("State Name") + ylab("Average Rent") + scale_fill_gradient(low="blue", high="red")
```
